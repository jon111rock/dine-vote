    ...
    <div class="w-full max-w-md">
      ...
      <div class="rounded-xl shadow-xl p-8 mb-6 backdrop-blur-md bg-white/80">
        ...
        <div class="flex border-b border-gray-200 mb-6">
          <button @click="switchTab('login')" :class="`cursor-pointer flex-1 text-center py-3 font-medium ${activeTab === 'login' ? 'border-b-2 border-red-500 text-red-500' : 'text-gray-500'}`">
            登入
          </button>
          <button @click="switchTab('register')" :class="`cursor-pointer flex-1 text-center py-3 font-medium ${activeTab === 'register' ? 'border-b-2 border-red-500 text-red-500' : 'text-gray-500'}`">
            註冊
          </button>
        </div>

        <!-- 登入表單 -->
        <form v-if="activeTab === 'login'" @submit="handleLogin">
          <div class="mb-4">
          ...
          <button type="submit" class="w-full text-white font-medium py-3 px-4 rounded-lg transition-all transform hover:-translate-y-1 hover:shadow-lg" style="background: linear-gradient(to right, #FF8A80, #FF5252);">
            登入
          </button>
        </form>
        ...
        <div class="grid grid-cols-2 gap-4">
          <button @click="handleSocialLogin('Google')" class="flex items-center justify-center py-2.5 px-4 rounded-lg border border-gray-300 bg-white hover:shadow-md hover:-translate-y-1 transition-all">
            <DIcon type="google" :size="5" class="mr-2" />
            <span class="text-gray-700 font-medium">Google</span>
          </button>
          <button @click="handleSocialLogin('Facebook')" class="flex items-center justify-center py-2.5 px-4 rounded-lg border border-gray-300 bg-white hover:shadow-md hover:-translate-y-1 transition-all">
      ...
      <div class="text-center text-sm text-gray-600">
        <template v-if="activeTab === 'login'">
          還沒有帳號？<a @click="switchTab('register')" href="javascript:void(0)" class="text-indigo-600 font-medium hover:text-indigo-800">立即註冊</a>
        </template>
        <template v-else>
          已有帳號？<a @click="switchTab('login')" href="javascript:void(0)" class="text-indigo-600 font-medium hover:text-indigo-800">立即登入</a>
        </template>
      </div>
    </div>
  </div>
